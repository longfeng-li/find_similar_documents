...
for i in range(len(partition)):
    ...
    for j in range(i):
        """
	sum = 0
	i_rdd = sc.parallelize(partition[i].maxweight)
	j_rdd = sc.parallelize(partition[j].maxweight)
	i_j = i_rdd.join(j_rdd)
	i_j_rdd = i_j.map(mul)
	for k in range(len(i_j_rdd.collect())):
		sum = sum + i_j_rdd.collect()[k][1]
		print sum 
        """
	sum = 0
